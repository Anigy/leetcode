...
public class P743NetworkDelayTime {
    ...
    public static int networkDelayTime(int[][] times, int N, int K) {
        PriorityQueue<Node> pq = new PriorityQueue<>((Comparator.comparing((Node n) -> n.distance)));
        ...
        for (int i = 0; i < times.length; i++) {
            ...
            Node des = map.get(time[1]);
            if (source.no == K) {
                des.distance = time[2];
            }
            source.adjacency.put(time[1], time[2]);
        }
        while (!pq.isEmpty()) {
            Node node = pq.poll();
            ...
            for(int v : node.adjacency.keySet()) {
                Node neighbor = map.get(v);
//                neighbor.distance = Math.min(neighbor.distance, node.adjacency.get(v) + node.distance);
                int newDis = node.adjacency.get(v) + node.distance;
                if(newDis < neighbor.distance) {
                    neighbor.distance = newDis;
                    pq.remove(neighbor);
                    pq.add(neighbor);
                }
            }
